...
/**
 * Performance test for the Claimant entity.
 */
class ClaimantGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Claimant entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all claimants")
            .get("/factcheck/api/claimants")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new claimant")
            .post("/factcheck/api/claimants")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created claimant")
                .get("/factcheck${new_claimant_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created claimant")
            .delete("/factcheck${new_claimant_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
